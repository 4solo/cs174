...
void drawBeeleg(float z,float x){
	int n_segment = 2;
	int amplitude = 5;
	float l_segment = 1.0;
	
		//model_view *= RotateY(10.0*TIME);
		model_view *= Translate( x,- l_segment / 2, 1.3f + z);
		for (int i = 0; i < n_segment; i++){
		model_view *= RotateZ(30/(i+1)* x*(-std::abs(sin(30 * DegreesToRadians*TIME))));
		model_view *= Translate(0, -l_segment / 2, 0);
		mvstack.push(model_view);
		model_view *= Scale(0.3, l_segment, 0.3);
		set_colour(0.741f, 0.718f, 0.420f);
		drawCube();
		model_view = mvstack.top();
		mvstack.pop();
		model_view *= Translate(0, -l_segment / 2, 0);
...
void drawBee()
{
	mvstack.push(model_view);  //head
	model_view *= RotateY(10.0*TIME);
	model_view *= Translate(15.0f, 10.0+5 * sin(100 * DegreesToRadians*TIME), 0.0f);
	mvstack.push(model_view);
	set_colour(0.482f, 0.408f, 0.933f);
	model_view *= Scale(1, 1, 1);
	drawSphere();
	model_view = mvstack.top();
	mvstack.pop();

	mvstack.push(model_view); //body
	//model_view *= RotateY(10.0*TIME);
	model_view *= Translate(0.0f,0.0f, 2.0f);
	set_colour(0.502f, 0.502f, 0.502f);
	model_view *= Scale(1, 1, 2);
	drawCube();
	model_view = mvstack.top();
	mvstack.pop();

	mvstack.push(model_view); //left wings
	//model_view *= RotateY(10.0*TIME);
	model_view *= Translate(-0.5f, 0.5f, 2.0f);
	model_view *= RotateZ(50 * sin(100 * DegreesToRadians*TIME));
	model_view *= Translate(-1.0f,0.0f, 0.0f);
	set_colour(0.878f, 1.0f, 1.0f);
	model_view *= Scale(2, 0.1, 1);
	drawCube();
	model_view = mvstack.top();
	mvstack.pop();

	mvstack.push(model_view); //right wings
	//model_view *= RotateY(10.0*TIME);
	model_view *= Translate(0.5f, 0.5, 2.0f);
	model_view *= RotateZ(-50.0*sin(100 * DegreesToRadians*TIME));
	model_view *= Translate(1.0f, 0.0f, 0.0f);
	set_colour(0.878f, 1.0f, 1.0f);
	model_view *= Scale(2, 0.1,1);
	drawCube();
